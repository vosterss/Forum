...
/*function post_recent_categorie($bdd){
	$reponse = $bdd->prepare('SELECT uti.id, uti.username, po.catégorie, po.contenu, po.titre, po.nb_like, po.date_publication from utilisateur as uti inner join post as po on uti.id = po.id_utilisateur AND po.categorie = ? AND po.date_publication = (SELECT MAX(date_publication) from post);');
	$reponse->execute(array());
	$result = $reponse->fetchAll();
	return $result;
}*/
...
/*function post_populaire_categorie($bdd){
	$reponse = $bdd->prepare('SELECT uti.id, uti.username, po.id, po.catégorie, po.contenu, po.titre, po.date_publication, po.nb_mess FROM utilisateur as uti inner join post as po on uti.id = po.id_utilisateur AND po.categorie = ? And po.nb_mess = (SELECT max(nb_mess) from post)');
	$reponse->execute(array());
	$result = $reponse->fetchAll();
	return $result;
}*/
